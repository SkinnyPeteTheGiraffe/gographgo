...
// ChannelsFromCheckpoint initializes a channel map from a persisted snapshot.
//
// For each key in the snap, the corresponding channel from the schema is asked to
// restore itself via FromCheckpoint. Keys in the snap that have no schema entry
...
// CreateCheckpoint serializes the current channel map into a snapshot map
