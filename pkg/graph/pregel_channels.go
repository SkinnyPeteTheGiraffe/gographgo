...
// Each channel in schema is copied so that this run's mutations are isolated.
...
// getOrCreate returns the channel for key, auto-creating a LastValue channel
// if no channel exists for that key. This matches LangGraph's behaviour of
...
// writeToChannel appends value to the pending writes list for channel key and
...
// restoreFromCheckpoint initialises a channel map from a previously saved
...
//     channel, matching LangGraph's map_input fall-through behaviour.
